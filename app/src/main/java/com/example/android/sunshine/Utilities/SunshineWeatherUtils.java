...
/**
 * Contains useful utilities for a weather app, such as conversion between
 * Celsius and Fahrenheit, from kph to mph, and from degrees to NSEW.
 * It also contains the mapping(отображение)of weather condition(погодных условий)
 * codes in OpenWeatherMap to strings.  These strings are contained
 */
public final class SunshineWeatherUtils {

    private static final String LOG_TAG =
            SunshineWeatherUtils.class.getSimpleName();
     ...
     */
    private static double celsiusToFarenheit(double temperatureInCelsius) {
        double temperatureInFarenheit = (temperatureInCelsius * 1.8) + 32;
        return temperatureInFarenheit;
    }

    /**
     * Temperature data is stored in Celsius by our app.
     * Depending on the user's preference, the app may need to display the temperature in Fahrenheit.
     * This method will perform(выполнить) that
     * temperature conversion(преобразование температуры)if necessary(необходимо).
     * It will also format the temperature so that no
    ...
    /**
     * This method uses the wind direction in degrees to determine compass direction
     * as a String. (eg NW) The method will return the wind String in the following form:
     * "2 km/h SW"
     *
     * @param context   Android Context to access preferences and resources
